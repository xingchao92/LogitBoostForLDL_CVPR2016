function [] = main()
    ...
    for fold=1:1
        save foldresult_expression foldresult fold;
        clear
        load foldresult_expression;
...
function [F,p,g,scores] = train(trainNum,trainFeature,trainData,trainFeatureNum,trainDataNum,itnum,v)
    ...
    for m=1:itnum 
        ...
        for j=1:trainDataNum
            ...
            g{m,j}=fitrtree(trainFeature(:,:),z(:,j),'Weights',w(:,j),'MinParent',int16(trainNum)/2);
            g{m,j}=compact(g{m,j});
        end
...
function [result] =  weak_cal(weak_tree,trainNum,trainFeature,trainDataNum,cur)
    result = zeros(trainNum,trainDataNum);
    for j=1:trainDataNum
        result(:,j)=predict(weak_tree{cur,j},trainFeature);
    end
end

function [sumv] =  BF(p,q)
    [m,n] = size(q);
    sumv = 0;
    for i = 1:m
        for j = 1:n
            if q(i,j) <= 0 || p(i,j) <= 0
                ...
                sumv = sumv + p(i,j)*log(p(i,j)/q(i,j));
            end
        end
    end
end