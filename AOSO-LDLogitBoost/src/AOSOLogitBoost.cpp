  ...
  AOSODatagain* _dt, VecIdx& vidx, int& c1, int& c2 )
{
  ...
  for (VecIdx::iterator it = vidx.begin(); it!=vidx.end(); ++it  ) {
    ...
    for (int k = 0; k < K; ++k) {
      ...
      mg[k] += ( (yi==k)? (train_data[idx][k]-pik):(train_data[idx][k]-pik) );
    } // for k
  } // for it
...
void AOSOSolver::update_internal( VecIdx& vidx )
{
  ...
  for (VecIdx::iterator it = vidx.begin(); it!=vidx.end(); ++it  ) {
    ...
    mg_[0] += ( (yi==cls1_)? (train_data[idx][cls1_]-p1):(train_data[idx][cls1_]-p1) );
    h_[0]  += ( p1*(1-p1) );

    mg_[1] += ( (yi==cls2_)? (train_data[idx][cls2_]-p2):(train_data[idx][cls2_]-p2) );
    h_[1]  += ( p2*(1-p2) );
...
const double AOSOLogitBoost::MAXF = 1000;
void AOSOLogitBoost::train( MLData* _data )
{
  train_init(_data);
  std::ifstream ifs("train_data.txt");
  int m=0,n=0;
  ifs >> m >> n;
  ofs<<m<<" "<<n<<endl;
  for (int i = 0; i < m; ++i){
  ...
  for (int i = 0; i < N; ++i) {
    ...
    L_.at<double>(i) = (-log(pik));*/
        ...
        for (int k = 0; k < K_; ++k) {
            ...
            double pik = *(ptr + k);
            if (pik<PMIN) pik = PMIN;
            if (train_data[i][k]<PMIN) train_data[i][k] = PMIN;
            //L_.at<double>(i) += (-train_data[i][k]*log(pik));
            
            L_.at<double>(i) += (train_data[i][k]*log(train_data[i][k]/pik));
            //ofs<<"L_.at<double>(i) =  "<<train_data[i][k]<<" "<<pik<<" "<<log(train_data[i][k]/pik)<<" "<<L_.at<double>(i)<<endl;
        }
	}
}
...
void AOSOLogitBoost::calc_grad( int t )
{
	...
	for (int i = 0; i < N; ++i) {
		...
		for (int k = 0; k < K_; ++k) {
			double pik = *(ptr_pi+k);
			if (yi==k) delta += std::abs( train_data[i][k]-pik );
			else       delta += std::abs( train_data[i][k]-pik);  
		}
	}
